...
logging = Logger(os.path.join(basedir,'all.log'),level='info').logger
class StockData:
    def __init__(self):
        # 获取stock接口
    ...
    def updateStocks(self):
        ...
        self.dbUtil.to_sql(data, 'stock_basic', if_exists='replace', index=False)
...
class LocalData:
    ...
    def data(self,code,type):
        ...
        if self.dbUtil.is_exist('stock_%s_%s_%s'%(category,type,code)):
            # 获取现有数据
            ...
            if not data.empty: # df[0:-1] 过滤掉开始时间对应的旧值
                ...
                self.dbUtil.to_sql(data, 'stock_%s_%s_%s' % (category,type,code), if_exists='append', index=False)
            return df#data.sort_values(by="trade_date",ascending=True)
        else: # 获取所有数据
            return self._init_data(code,zs,type)
    # 根据代码获取深沪类型、编号、指数信息
    ...
    def codes(self,type=None):
        if type is None: # 默认展示所有股票
            ...
            date = self.dbUtil.read_sql('select max(rq) rq from  (select rq,COUNT(1) num from stock_sdgd  group by rq) where num>3000')
            rq = date['rq'].values[0]
    ...
    def test_result(self,key,*args,**kwargs):
        ...
        if self.dbUtil.is_exist('stock_%s_result'%category):
            sql = "select * from (select c.name 名称,sr.code 代码,sr.bj 本金,sr.ztsy 总体收益,sr.jycs 交易次数,sr.jf 缴费,sr.cgl 成功率 ,c.industry ||','|| c.area ||'板块,'||GROUP_CONCAT(sbg.blockname) 板块 " \
                  "from stock_%s_result sr,stock_basic c ,stock_bk_gn sbg where c.symbol=sr.code and  sbg.code  = c.symbol  and key='%s' " \
                  "group by c.symbol )" \
                  "WHERE 代码 like '%%%s%%' and 名称 like '%%%s%%' and 板块 like '%%%s%%'" \
                  "order by 成功率 desc,总体收益 desc"%(category,key,kwargs['code'],kwargs['name'],kwargs['bk'])
    ...
    def test_order(self,key,code):
        ...
        if self.dbUtil.is_exist('stock_%s_order'%category):
            return self.dbUtil.read_sql("select trade_date 日期,type 类型, close 价格,ss 手数,cje 成交额,kc 扣除费 from stock_%s_order where code='%s' and key='%s'" %(category,code,key))
        return None
    ...
    def saveOrder(self,order,result):
        ...
        self.dbUtil.to_sql(df,'stock_%s_order'%category,if_exists='append',index=False)
        self.dbUtil.to_sql(dfr,'stock_%s_result'%category,if_exists='append',index=False)
    # 初始化数据
    def _init_data(self,code,zs,type):
        ...
        if not data.empty:
            ...
            self.dbUtil.to_sql(tmp, 'stock_%s_%s_%s' % (config.getByKey('TDX_CATEGORY'),type,code), if_exists='replace', index=False)
    ...
    def update_stock_pool(self, datas=None,handle=0):
            ...
            if handle==1:# 当日成交时触发
                for data in datas:
                    ...
                    if code in df['code'].values:
                        # 修改df中对应编号的状态及数量
                        df.loc[df['code'] == code, 'state'] = data['state']
                        df.loc[df['code'] == code, 'num'] = data['num']
                        continue
                    temps.append({'code': code, 'state': data['state'], 'num': data['num'], 'seq': '0'})
                if len(temps)>0:
        ...
        self.dbUtil.to_sql(df, 'stock_buy_pools', if_exists='replace', index=False)
    @classmethod
    def getInstance(cls):
...
class TdxData:
    ...
    def updateBk(self):
        with self.api.connect(TDX_IP,TDX_PORT):
           bk_zs = self.api.to_df(self.api.get_and_parse_block_info("block_zs.dat"))#指数板块
            ...
            self.dbUtil.to_sql(bk_gn, 'stock_bk_gn', if_exists='replace', index=False)
            self.dbUtil.to_sql(bk_fg, 'stock_bk_fg', if_exists='replace', index=False)
            self.dbUtil.to_sql(bk_zs, 'stock_bk_zs', if_exists='replace', index=False)
            self.dbUtil.to_sql(bk_default, 'stock_bk_default', if_exists='replace', index=False)
            ...
            self.dbUtil.to_sql(tmp, 'stock_bk', if_exists='replace', index=False)
    def updateGD(self,code,type):
        ...
        try:
            ...
            pass
        self.dbUtil.to_sql(df_gdrs, 'stock_gdrs', if_exists='append', index=False)
        sdgd = []
        ...
        try:
            ...
            pass
        self.dbUtil.to_sql(df_sdgd, 'stock_sdgd', if_exists='append', index=False)
        sdltgd = []
        ...
        try:
            ...
            pass
        self.dbUtil.to_sql(df_sdltgd, 'stock_sdltgd', if_exists='append', index=False)
    ...
    def updateFhYears(self):
            i = time.strftime("%Y", time.localtime())
        # for i in range(1991,2020):#初始化时开启
...
class DBUtil:
    ...
    def to_sql(self,df,tablename,if_exists='fail',index=True):
        self.instance.to_sql(df,tablename,if_exists=if_exists,index=index)
        return 1
    ...
    @classmethod
    def getInstance(cls,type='msyql'):
        if not hasattr(DBUtil, "_instance"):
            DBUtil._instance = DBUtil(type)
...
class DBUtil_Mysql:
    ...
    def to_sql(self,df,tablename,if_exists='fail',index=True):
        pd.io.sql.to_sql(df,tablename,self.engine,if_exists=if_exists,index=index)
        return 1
    def execute_sql(self,sql,*args):
...
class DBUtil_Sqlite:
    ...
    def read_sql(self,sql):
        return pd.read_sql(sql,self.connection)
    def to_sql(self,df,tablename,if_exists='fail',index=True):
        pd.io.sql.to_sql(df,tablename,self.connection,if_exists=if_exists,index=index)
        return 1
    def execute_sql(self,sql,*args):
        cursor = self.connection.cursor()
        cursor.execute(sql, *args)
